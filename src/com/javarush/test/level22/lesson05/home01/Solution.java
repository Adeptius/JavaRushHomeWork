...
public class Solution {
    ...
    public String getPartOfString(String string, String threadName) {
        String result = null;
        try{ result = string.substring(string.indexOf("\t")+1, string.lastIndexOf("\t"));  }
        catch (Exception e){
            if     (FIRST_THREAD_NAME.equals(threadName)) { throw new TooShortStringFirstThreadException(e);  }
            else if(SECOND_THREAD_NAME.equals(threadName)){ throw new TooShortStringSecondThreadException(e); }
